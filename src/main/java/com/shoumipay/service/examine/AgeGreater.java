 ...
 */
public class AgeGreater extends BasicExamineService {

    private MemberDetailService memberDetailService;

    private Integer ageLine;

    public AgeGreater(ExamineRule examineRule, MemberDetailService memberDetailService, OperLogsService operLogsService) {
    ...
    public int examine(Member member) {
        ...
        logger.info("AgeGreater 不通过 --> " + examineRule + "\n" + memberDetail);
        operLogsService.addOrderExamineLog("审核不通过", "AgeGreater 不通过 --> " + examineRule + "\n" + memberDetail, 3, 0L);
