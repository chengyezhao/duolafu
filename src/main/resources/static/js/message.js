    ...
    $(function () {
        ...
        $(grid_selector).jqGrid({
            ...
            colModel: [
                ...
                { label: '收件人', name: 'acceptor', width: 100},
                { label: '时间', name: 'createTime', width: 100},
                { label: '操作人', name: 'processor', width: 100}
        ],
        ...
        $("#queryBtn").click(function () {
            ...
            $(grid_selector).jqGrid('setGridParam', {
                postData: {startTime: startTime, endTime: endTime,types:types,process:process},
                //search: true,
                page: 1
            }).trigger("reloadGrid");
        });
    ...
    //修改标题，弹出修改窗
    $("#modifyMessageBtn").click(function () {
    ...
    $("#deleteMessageBtn").click(function () {
        var rows=$(grid_selector).getGridParam('selarrrow');
         if(rows.length>1){
            // $.messager.alert("温馨提示","不能同时修改多条记录！");
            layer.msg('不能同时删除多条记录！', {icon: 7,time: 2000}); //2秒关闭（如果不配置，默认是3秒）
            return;
        }
        if(rows.length>0){
            $.messager.confirm("温馨提示", "是否确定删除所选记录？", function() {
                var ids = $(grid_selector).jqGrid('getRowData', rows[0]).id;
                $.ajax({
                    url:"notice/delete",
                    cache: false,
                    type:"post",
