    ...
    $(function () {
        ...
        $(grid_selector).jqGrid({
            url: "/risk/examine/queryExamineRuleList",
            datatype: "json",
            mtype: 'POST',
            height: window.screen.height - 550,
            colModel: [
                { label: 'id', name: 'id', width: 75, hidden: true},
                { label: '风控类型', name: 'type', width: 200},
                { label: '风控参数值', name: 'parameter', width: 100 /*,formatter: function (cellvalue, options, cell) {
                    var stateStr = "发送成功";
                ...
                { label: 'match', name: 'match', width: 100 },
                { label: 'unmatch', name: 'unmatch', width: 100 },
                { label: '操作人', name: 'processor', width: 100,formatter: function (cellvalue, options, cell) {
                    var stateStr = "admin";
                    if (cell.processor == 9) {
                        stateStr = "admin";
                    } else {
                        stateStr = cell.processor;
                    }
                    return stateStr;
                } },
                { label: '操作时间', name: 'modifyTime', width: 100 },
                { label: '是否启用', name: 'using', width: 100, formatter: function (cellvalue, options, cell) {
                    var stateStr = "发送成功";
                    if (cell.using == 0) {
                        stateStr = "未启用";
                    } else {
    ...
    //修改标题，弹出修改窗
    $("#modifyExamineRuleBtn").click(function () {
        ...
        if(rows==0){
            ...
            $("#id").val(data.id);
            $("#type").val(data.type);
           // $("#use12").val(data.using);
            $("#parameter").val(data.parameter);
            $("#match").val(data.match);
            $("#unmatch").val(data.unmatch);
            $('#myModalLabel').text('修改风控参数模版');
            $("#addModal").modal({
                keyboard : false,
                show : true,
                backdrop : "static"
...
function saveSmsTemp(){
    ...
    var using = $('#use12').val();
    var match = $('#match').val();
    var unmatch = $('#unmatch').val();
    $.ajax({
        url: "/risk/examine/"+task,
        cache: false,
        dataType:'json',
        data : {
            "id":id,
